...
module ActiveRecord
  class WhereTest < ActiveRecord::TestCase
    ...
    def test_has_many_scoped_where
      expected = Author.where(hello_posts_with_hash_conditions: Post.where(id: [1,2]))
      actual   = Author.where(posts: Post.where(body: 'hello', id: [1,2]))
    ...
    def test_has_many_through_array_where
      child = Comment.new
      child.post_id = 1
    ...
    def test_has_many_through_empty_array_where
      expected = Author.where(comments: [])
      actual   = Author.where(posts: Post.where(id: []))

      assert_equal expected.to_sql, actual.to_sql
    end

    def test_has_many_through_array_with_nil_where
      expected = Author.where(comments: [nil])
      actual   = Author.where.not(posts: Post.where(comments: Comment.all))
    ...
    def test_has_many_through_array_with_record_where
      child = Comment.new
      child.post_id = 1
    ...
    def test_has_many_through_array_with_primary_key_where
      expected = Author.where(comments: [1])
